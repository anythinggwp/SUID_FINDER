...
sudo lxc-create -t astralinux-se -n "$CONTNAME"
sudo lxc-start -n "$CONTNAME"
...
run_in_container "$CONTNAME" "apt-get update && apt-get install -y build-essential gcc g++ make flex bison libgmp-dev libmpfr-dev libmpc-dev texinfo wget git time"
run_in_container "$CONTNAME" "mkdir -p $BUILD $LOGS /opt/gcc-test-build"
sudo lxc-stop -n "$CONTNAME"

echo "Копируем исходники gcc в контейнер"
sudo rsync -av ~/git/gcc/ /var/lib/lxc/${CONTNAME}/rootfs/root/gcc/
sudo lxc-start -n "$CONTNAME"
